...
## Tình huống — Long tạo account GitHub lần đầu

Sau bộ Git ([5 lesson Long story](../../01_Foundations/version-control/git/lessons/01_basic/)), Long đã quen với `git init`, `commit`, `branch`. Sếp bảo *"push lên GitHub đi"*. Long vào github.com:

> *Sign up → đặt username gì? `long123`? `nguyen-van-long`? Có thay đổi sau được không?*
...
5 câu hỏi đầu tiên. Bài này dẫn Long (và bạn) qua **mọi setup cần thiết** trong 30 phút, từ account → push xong → setup PR workflow với Mai → publish portfolio Pages → tự động CI/CD qua Actions.

→ Cuối bài Long có **GitHub setup production-ready**, dùng cho mọi project tiếp theo.
...
2. Email + password + **username**:
   ...
   - Khuyến nghị: dùng tên thật (nguyenvanlong), không số random (user12345). Recruiter tin tưởng tên thật hơn.
3. Verify email
...
## 6️⃣ Pull Request Workflow — Long + Mai collab

Workflow chuẩn khi 2+ người làm chung. Tiếp Long story:

### Setup branch protection (1 lần)

Trước khi team làm, Long set rule cho `main`:

1. Settings → **Branches** → **Add rule**
2. **Branch name pattern**: `main`
3. ✅ Require a pull request before merging
4. ✅ Require approvals: **1** (Mai approve mới merge được)
5. ✅ Require status checks (CI pass)
6. ✅ Do not allow bypassing (kể cả admin)
7. Save

→ Giờ KHÔNG ai (kể cả Long admin) push thẳng vào `main`. Phải qua PR.

### Mai làm feature mới qua PR
...
remote:      https://github.com/long/myapp/pull/new/feature/add-search
```
...
- **Review changes** (top right):
   ...
   - **Approve** ✅ — OK merge
   - **Request changes** ❌ — chưa OK, Mai phải sửa

### Mai sửa theo feedback
...
```
✅ build (passed)
❌ test (failed)        ← Mai phải sửa
✅ lint (passed)
```

→ Reviewer thấy đỏ → reject. Mai phải fix test.
...
- 🎓 [Git lesson 03 — Remote + GitHub](../../01_Foundations/version-control/git/lessons/01_basic/03_remote-and-github.md) — bài lesson Mai join project (Long story)
...
- **v1.0.0 (23/05/2026)** — Bản đầu tiên — full user guide GitHub. 11 phần lớn: tình huống Long tạo account → §1 GitHub là gì + ẩn dụ mạng xã hội cho dev → §2 Account + 2FA → §3 Auth 3 cách (SSH/PAT/gh login) → §4 Tạo repo + push → §5 UI tour 8 tab → §6 PR workflow chi tiết với Long+Mai → §7 Actions CI/CD → §8 Pages portfolio → §9 `gh` CLI → §10 Security 7 must-do + 5 không được → §11 Settings nâng cao. 5 pitfall + 4 self-check + cheatsheet URL/shortcut/gh + glossary 17 thuật ngữ.
