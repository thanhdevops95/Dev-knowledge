...
### Azure SQL Database — mô hình mua (purchasing model)
...
### Geo-replication (read replica xuyên region)
...
### Auto-failover Group (khuyến nghị cho production)
...
Sơ đồ dưới rút gọn quyết định cốt lõi của cả bài thành vài câu hỏi: dữ liệu có cần quan hệ + ACID không, và có cần phân tán toàn cầu + ghi đa region không.

```mermaid
flowchart TD
    START["Workload cần DB"] --> Q1{"Quan hệ + ACID + transaction?"}
    Q1 -->|Có| Q2{"Lift-and-shift SQL Server cũ?"}
    Q2 -->|Có| MI["Azure SQL Managed Instance"]
    Q2 -->|Không| SQL["Azure SQL Database (GP/BC/Hyperscale)"]
    Q1 -->|Không, NoSQL| Q3{"Cần global + low-latency + multi-region writes?"}
    Q3 -->|Có| COSMOS["Cosmos DB (multi-API)"]
    Q3 -->|Không, single-region| SERVERLESS["Cosmos DB Serverless / JSON columns"]
```

Tóm lại: cần ACID quan hệ thì rẽ nhánh Azure SQL, còn cần độ trễ thấp xuyên lục địa thì rẽ nhánh Cosmos DB.
...
- **v2.0.2 (11/06/2026)** — Bổ sung sơ đồ decision flow chọn Azure SQL vs Cosmos DB cho trực quan.
