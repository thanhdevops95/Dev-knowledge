...
Sơ đồ dưới cho thấy hai luồng tách biệt cùng dùng chung một bucket: luồng sự kiện (upload → Function resize) và luồng đọc (mobile → Gateway → Cloud Run):

```mermaid
flowchart LR
    user["User upload"] --> gcs["GCS: originals/"]
    gcs -->|"event finalized"| fn["Cloud Function (resize)"]
    fn --> resized["GCS: resized/"]
    mobile["Mobile app"] --> gw["API Gateway (auth + rate limit)"]
    gw --> run["Cloud Run API"]
    run --> resized
```

Điểm mấu chốt: Cloud Function chạy theo *sự kiện* (kích hoạt khi có file mới), còn Cloud Run phục vụ *request* (mobile gọi qua Gateway) — hai mô hình tính toán khác nhau ghép quanh cùng một kho dữ liệu.
...
- **v2.0.1 (11/06/2026)** — Bổ sung sơ đồ pipeline hands-on (luồng sự kiện GCS→Function và luồng request Mobile→Gateway→Cloud Run) cho trực quan.
