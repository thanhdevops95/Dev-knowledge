...
Bốn ý trên dễ hình dung hơn khi nhìn cả cây phân cấp cùng hướng kế thừa: quyền IAM cấp ở tầng trên tự "chảy" xuống mọi tầng dưới, còn billing thì gắn vào từng project.

```mermaid
flowchart TD
    O["Organization"] -->|"IAM kế thừa xuống"| F["Folder"]
    F -->|"IAM kế thừa xuống"| P["Project"]
    P --> R["Resource (VM, bucket, DB)"]
    B["Billing account"] -.->|"gắn vào"| P
```

Sơ đồ cho thấy vì sao cấp quyền ở tầng Organization là "quyền lực nhất" (lan xuống mọi project bên dưới), còn muốn cô lập chi phí và quyền hạn thì phải tách ngay ở tầng Project.
...
- **v2.0.2 (11/06/2026)** — Bổ sung sơ đồ phân cấp Org → Folder → Project (kế thừa IAM + billing) cho trực quan.
