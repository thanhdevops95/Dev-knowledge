...
OK lệnh chạy. Nhưng:
1. `/etc` là **folder gì**? Sao nó ở **gốc** chứ không phải trong home (`/Users/rom`)?
...
🪞 **Ẩn dụ**: filesystem giống **cây gia phả lộn ngược**. Gốc ở **trên cùng** (root `/`), nhánh **toả xuống dưới** (folder con, cháu, chắt). Mỗi file/folder có **địa chỉ duy nhất** từ gốc xuống (`/Users/rom/Desktop/myapp/main.py` = "đi từ gốc → Users → rom → Desktop → myapp → file main.py").
...
```
/Users/rom/Desktop/myapp/main.py
```

→ Đọc: "Từ ROOT → Users → rom → Desktop → myapp → file main.py".
...
→ Đọc: phụ thuộc bạn đang ở đâu. Nếu đang ở `/Users/rom/myapp/`:
- `./config.json` = `/Users/rom/myapp/config.json`
- `../backups/` = `/Users/rom/backups/`
...
| `~` | Tilde | Home folder user hiện tại | `~/Desktop` = `/Users/rom/Desktop` |
...
Đang ở `/Users/rom/projects/myapp/src/`. Diễn dịch các path:

| Lệnh | Nghĩa absolute |
|---|---|
| `cd ~` | → `/Users/rom` (home) |
| `cd ~/Desktop` | → `/Users/rom/Desktop` |
| `cd ..` | → `/Users/rom/projects/myapp/` (lên 1 cấp) |
| `cd ../..` | → `/Users/rom/projects/` (lên 2 cấp) |
| `cd ./tests` | → `/Users/rom/projects/myapp/src/tests/` |
| `cd /` | → `/` (về gốc) |
...
```mermaid
graph TD
    A[Bạn mở terminal] --> B[CWD = home folder<br/>vd /Users/rom]
    B --> C[Gõ cd Desktop]
    C --> D[CWD đổi sang /Users/rom/Desktop]
...
```bash
# Đang ở /Users/rom
ls                       # liệt kê file trong /Users/rom
python main.py           # tìm main.py trong /Users/rom — nếu không có → lỗi
cat config.json          # tìm config.json trong /Users/rom
```
...
```
-rwxr-xr-x  1 rom  staff   542 May 16 main.py
```
...
lrwxr-xr-x  1 rom  staff   34 May 16 /usr/local/bin/python -> /opt/homebrew/bin/python3.11
```
...
### ❌ Pitfall: Lẫn lộn `~` với root `/`

```bash
cd ~         # → /Users/rom (home — đúng)
...
### ❌ Pitfall: Quên CWD → chạy lệnh sai context

```bash
# Đang ở /Users/rom (chưa cd vào project)
...
### ❌ Pitfall: Path có space chưa escape
...
├── myapp/
│   ├── .git/           ← git track
│   ├── .gitignore      ← KEY — list .env, node_modules, ...
│   ├── .env            ← ignored by git ✓
│   ├── src/
│   └── README.md
└── other-app/
...
**Q2.** Bạn đang ở `/Users/rom/projects/myapp/`. Gõ `cd ../../Desktop`. Bạn đến folder nào?
...
Phân rã:
- `..` → lên 1 cấp = `/Users/rom/projects/`
- `..` → lên 1 cấp nữa = `/Users/rom/`
- `Desktop` → vào folder Desktop = `/Users/rom/Desktop`
...
| 📚 Git `.gitignore` (tránh leak `.env`) | [`01_Foundations/version-control/git/lessons/01_basic/01_init-and-first-commit.md`](../../../version-control/git/lessons/01_basic/01_init-and-first-commit.md) §3 |
