...
- ❌ Code mẫu chưa test
- ❌ Heading tiếng Anh trong bài VN (heading kỹ thuật phải Việt hoá, giữ EN trong ngoặc)
- ❌ Mọi ước tính thời gian (reading-time per-bài, "X tháng/tuần/giờ" cấp roadmap/stage, "~X phút làm bài") — kho không dùng con số thời gian
...
| Section Cạm bẫy (Pitfall) | Optional | Often Required |
| Section Tra cứu nhanh (Cheatsheet) | Không | Often Yes |
| Section Từ điển thuật ngữ (Glossary) | Yes (concept gốc) | Yes (terms trong bài) |
...
├── 📄 kind.md                       ← cài Kind
├── 📄 install-docker.md             ← cài Docker (runtime nền cho K8s local)
└── ...
```
...
├── 📦 01_<project-name>/
│   ├── README.md                    ← intro project, mục tiêu, yêu cầu trước
│   ├── 00_setup.md
│   ├── 01_<step-1>.md
│   ├── 02_<step-2>.md
│   ├── ...
...
```markdown
# Tra cứu nhanh (Cheatsheet) — <Topic>
...
```markdown
# Từ Điển Thuật Ngữ (Glossary) — <Topic>
...
Menu 7 loại lõi giới thiệu ở §0.2 của `01_sitemap-detail.md`; danh sách loại bổ sung dưới đây do chính file này (§4) làm canonical owner. Khi cần loại mới:
...
> 📌 **Quy ước bullet**: trong toàn kho, danh sách không thứ tự dùng dấu gạch ngang `-` (1 space sau dấu), KHÔNG dùng `*`. Nested list thụt 2 space.
...
> 📌 **Quy ước repo**: changelog trong kho `dev-knowledge` xếp **tăng dần** (cũ → mới, bản mới nhất ở cuối list). Quy ước này **override** global skill (global mặc định reverse-chronological — bản mới nhất trên cùng).
...
- **v1.0.0 (26/05/2026)** — **Tái cấu trúc lớn (Gộp file & Quy chuẩn Cheatsheet)**:
  ...
  - Đổi quy chuẩn đặt tên file cheatsheet từ `cheatsheet.md` thành `_cheatsheet.md` (thêm prefix `_`) xuyên suốt tài liệu và sơ đồ cây.
- **v1.1.0 (01/06/2026)** — **Đồng bộ 3 quyết định governance + alignment quy ước nền**:
  - (A) Changelog đổi sang **tăng dần** (cũ → mới) + heading "## 📌 Nhật ký thay đổi (Changelog)" + ghi chú override global skill. Lý do: thống nhất chiều đọc changelog toàn repo.
  - (B) Việt hoá heading kỹ thuật (giữ EN trong ngoặc): mẫu `# Tra cứu nhanh (Cheatsheet)`, `# Từ Điển Thuật Ngữ (Glossary)`, header bảng glossary `| Thuật ngữ | Tiếng Việt | Giải thích |`, nhãn section trong bảng Intro-vs-Lesson. Lý do: nhất quán heading-style toàn repo.
  - (C) Bỏ hết ước tính thời gian: gỡ "thời gian ước tính" ở README `exercises/`; đổi anti-pattern CONTRIBUTING thành cấm mọi con số thời gian. Lý do: kho không dùng reading-time / "X tháng/tuần/giờ".
  - Alignment: chuyển toàn bộ bullet `*` → `-` (§3.1.1 + §5.0–5.8) + thêm quy ước bullet dash vào §5.1.5; `editor/` → `ide/` (khớp §3.2bis); ví dụ setup `docker-desktop.md` → `install-docker.md`; sửa tautology changelog v1.0.0; sửa §4 tham chiếu §0.5 (đã gộp) → trỏ §0.2 sitemap + ghi rõ §4 là canonical owner; field "Prerequisites" → "Yêu cầu trước". Lý do: gỡ tham chiếu chết + thống nhất quy ước nền.
