...
|---|---|---|
| [`README.md`](./README.md) | File bạn đang đọc — cổng tổng quan của Blueprint | v1.1.0 |
| [`_CONCEPT-MAP.md`](./_CONCEPT-MAP.md) | **SSOT register** — concept nào ở file nào (tra trước khi sửa) | v1.1.0 |
| [`01_sitemap-detail.md`](./01_sitemap-detail.md) | Sitemap mở rộng L1 → L2 → L3 của toàn bộ kho | v0.5.0 |
| [`02_folder-structure.md`](./02_folder-structure.md) | Cấu trúc folder/file chuẩn + Quy ước đặt tên + hub setup | v1.1.0 |
| [`03_writing-style.md`](./03_writing-style.md) | Chuẩn viết: WHY→WHAT→HOW, ẩn dụ, emoji, **Quy chuẩn Alert Boxes** | v0.7.0 |
| [`05_linking-strategy.md`](./05_linking-strategy.md) | Link chéo, glossary, cross-L2 pattern | v0.3.0 |
| [`06_roadmap-design.md`](./06_roadmap-design.md) | Cách thiết kế roadmap, template, ví dụ | v0.3.0 |
| [`07_quality-checklist.md`](./07_quality-checklist.md) | Checklist kiểm tra trước khi publish bài | v0.4.0 |

### 📁 Thư mục con hỗ trợ

| Thư mục | Nội dung |
|---|---|
| [`templates/`](./templates/) | 10 template thật để copy-paste khi viết bài: `lesson_template.md`, `topic-readme_template.md`, `roadmap_template.md`, `overview_template.md`, `exercise_template.md`, `lab_template.md`, `recipe_template.md`, `setup_template.md`, `master-catalog_template.md`, `contributing_template.md` (kèm `README.md` chỉ dẫn) |
| [`examples/`](./examples/) | Ví dụ tham chiếu: 1 chủ đề L2 mẫu viết hoàn chỉnh (`sample_kubernetes-pod/`) + nhiều bài mẫu lẻ (lesson, short-lesson, exercise, recipe, roadmap-excerpt, tool-category, tool-individual) |

---

## 6️⃣ Quy ước chung trong các file Blueprint

> ⚠️ **Ranh giới:** Khung bố cục + metadata dưới đây **chỉ áp cho file Blueprint `0X_*.md`** (author-facing). File **bài học** trong các chủ đề L1 dùng khung riêng theo [`templates/lesson_template.md`](./templates/lesson_template.md) và [`03_writing-style.md`](./03_writing-style.md) — không áp khung của Blueprint vào bài học, và ngược lại.

Để Blueprint tự nhất quán với chính nó, các file `0X_*.md` đều tuân:

### Bố cục chuẩn (chỉ cho file Blueprint `0X_*.md`)
...
> 📌 **Changelog tăng dần toàn repo:** mọi file trong `dev-knowledge` (kể cả file Blueprint này) ghi changelog theo thứ tự **tăng dần** — dòng cũ nhất trên cùng (`v0.1.0`), dòng mới nhất nằm cuối. Đây là quy ước **override** global skill `naming/metadata-headers.md` (mặc định reverse-chronological). Heading thống nhất là `## 📌 Nhật ký thay đổi (Changelog)`.
...
> Repo `dev-knowledge` dùng changelog **tăng dần** (cũ → mới, dòng mới nhất nằm cuối danh sách) cho **toàn bộ file**, kể cả các file Blueprint. Đây là quy ước **override** global skill (`naming/metadata-headers.md` mặc định reverse-chronological / mới nhất trên cùng) — áp dụng riêng trong repo này để dễ đọc theo dòng thời gian phát triển.
...
- **v1.1.0 (01/06/2026)** — Đồng bộ governance theo 3 quyết định đã chốt: đổi changelog sang **tăng dần** toàn repo + ghi rõ override global skill; đính chính §5 (danh mục template thực tế 10 file, bỏ `_cheatsheet.md` không tồn tại; mô tả `examples/` đúng — 1 chủ đề L2 mẫu + nhiều bài mẫu lẻ); đồng bộ cột version theo file thực (`_CONCEPT-MAP` v1.0.0, `03_writing-style` v0.5.8); §6 ghi rõ ranh giới khung metadata chỉ áp cho file Blueprint `0X_*.md`. Lý do: bảng danh mục lệch thực tế gây nhầm cho người đóng góp, và cần chuẩn hoá quy ước changelog xuyên repo.
