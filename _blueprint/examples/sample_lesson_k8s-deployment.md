...
> **Level:** Basic\
> **Tags:** [MUST-KNOW]\
> **Yêu cầu trước:** [Pod — Đơn vị deploy nhỏ nhất của Kubernetes](../sample_kubernetes-pod/lessons/01_basic/01_pod.md) (ReplicaSet sẽ giải thích trong bài)
...
### ❌ Cạm bẫy: `selector.matchLabels` khác `template.metadata.labels`
...
### ❌ Cạm bẫy: quên set `resources.requests/limits`
...
- ⬅️ **Bài trước:** [Pod — Đơn vị deploy nhỏ nhất của Kubernetes](../sample_kubernetes-pod/lessons/01_basic/01_pod.md)
- ➡️ **Bài tiếp theo:** Service — expose Deployment ra ngoài (chưa có)
- ↑ **Về cụm:** [Kubernetes — README cụm](../sample_kubernetes-pod/README.md)

### 🧩 Các chủ đề có thể bạn quan tâm

- StatefulSet — Pod có identity ổn định (chưa có)
- HPA — auto-scaling Deployment theo CPU/RAM (chưa có)
...
- **v1.0.0 (16/05/2026)** — Bản đầu tiên — bài Deployment (WHY/WHAT/HOW + rolling update + rollback).
- **v1.1.0 (16/05/2026)** — Bổ sung §4 *Recreate strategy* + 3 use case cụ thể (DB migration, singleton service, resource-tight cluster); cập nhật note `maxSurge`/`maxUnavailable` mặc định = 25%.
- **v1.2.0 (01/06/2026)** — Đảo changelog sang thứ tự tăng dần (cũ → mới) + heading chuẩn; gỡ meta-leak nguồn nội bộ trong changelog; Việt hoá heading Pitfall/Self-check/Cheatsheet/Glossary; sửa link rỗng `[ReplicaSet](#)`; chuẩn hoá section Liên kết sang 3-sub + nav bullet (link text = tiêu đề thật); đổi "Prerequisites" → "Yêu cầu trước". Lý do: đồng bộ 3 quyết định governance + bài mẫu phải sạch meta-leak.
