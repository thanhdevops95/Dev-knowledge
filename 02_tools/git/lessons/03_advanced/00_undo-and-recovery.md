...
Để chọn nhanh đúng lệnh, hãy đi theo cây quyết định dưới — câu hỏi gốc luôn là "đã commit chưa", rồi mới tới "đã push chưa".

```mermaid
flowchart TD
    Q1{Đã commit chưa?} -->|Chưa, chỉ sửa file| R1["git restore <file>"]
    Q1 -->|Đã add, chưa commit| R2["git restore --staged <file>"]
    Q1 -->|Đã commit| Q2{Đã push lên remote chưa?}
    Q2 -->|Chưa push| R3["git reset (sửa local)"]
    Q2 -->|Đã push, người khác có thể đã pull| R4["git revert (an toàn)"]
    Q2 -->|Lỡ tay mất commit/nhánh| R5["git reflog → khôi phục"]
```

Mấu chốt: càng "đi xa" (đã push, người khác đã pull) thì càng phải chọn lệnh an toàn (`revert`) thay vì lệnh ghi đè lịch sử (`reset`).
