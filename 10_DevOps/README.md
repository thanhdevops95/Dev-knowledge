...
> 🚀 **Status:** Có bộ Docker đầu tiên ở [`docker/`](./docker/). Các L2 khác còn skeleton.

## 🎯 Chủ đề này có gì

DevOps stack: Docker, Kubernetes, CI/CD, IaC (Terraform, Ansible), Observability, GitOps, Service mesh — toàn bộ pipeline từ dev → deploy → operate.

## 📂 L2 chủ đề con

| L2 | Trạng thái | Note |
|---|---|---|
| [`docker/`](./docker/) | 🚀 5 bài | **Bộ Docker basic hoàn chỉnh** — setup + 4 lessons |
| `kubernetes/` | ❌ Chưa có | Container orchestration |
| `ci-cd/` | ❌ Chưa có | Jenkins, GitHub Actions, GitLab CI |
| `iac/` | ❌ Chưa có | Terraform, Ansible |
| `observability/` | ❌ Chưa có | Prometheus, Grafana, ELK |
| `gitops/` | ❌ Chưa có | ArgoCD, Flux |
| `service-mesh/` | ❌ Chưa có | Istio, Linkerd |
