...
User mở http://localhost → NGINX
NGINX serve index.html, style.css, app.js
Browser render giao diện
User click "Add TODO" → JavaScript gọi API
AJAX request → http://localhost/api/todos
NGINX proxy → Python (port 8080)
Python forward → Go (port 8081)
Response ngược → Go → Python → NGINX → Browser
JavaScript update DOM


🎨 PHẦN 2: XÂY DỰNG FRONTEND
2.1. Tạo cấu trúc thư mục
bash# Di chuyển về thư mục gốc
