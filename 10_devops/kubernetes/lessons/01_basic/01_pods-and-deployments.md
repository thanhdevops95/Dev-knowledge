...
Deployment thay từng Pod một (theo `maxSurge`/`maxUnavailable`), luôn còn Pod phục vụ traffic nên không gây downtime. Sơ đồ dưới đây tóm tắt 3 giai đoạn chuyển trạng thái:

```mermaid
flowchart LR
    subgraph Trước["Trước update (v1)"]
        A1[Pod v1] & A2[Pod v1] & A3[Pod v1]
    end
    subgraph Giữa["Đang update"]
        B1[Pod v2] & B2[Pod v1] & B3[Pod v1]
    end
    subgraph Sau["Sau update (v2)"]
        C1[Pod v2] & C2[Pod v2] & C3[Pod v2]
    end
    Trước --> Giữa --> Sau
```

Nhìn vào giai đoạn giữa, ta thấy luôn có Pod cũ v1 còn sống song song với Pod mới v2 nên dịch vụ không bao giờ bị gián đoạn.
