...
> **Level:** Basic  
> **Tags:** [MUST-KNOW]  
> **Yêu cầu trước:** Đã [Cài đặt Docker](../../setup/install-docker.md), biết [Git](../../../../02_tools/git/) và [Terminal](../../../../01_foundations/computing-environment/lessons/01_basic/00_what-is-terminal.md) cơ bản.
...
Sau khi hoàn thành bộ bài học Git, bạn vô cùng tự tin đẩy mã nguồn dự án của mình (Ex: `myapp`) lên GitHub. Chiều thứ Hai, một đồng nghiệp Frontend mới gia nhập đội ngũ bắt đầu thiết lập máy tính cá nhân để chuẩn bị code.
...
Quay trở lại tình huống trên, 3 lỗi đồng nghiệp gặp phải đều thuộc về một trạng thái kinh điển gọi là **"Dependency Hell" (Địa ngục thư viện)** — khi môi trường máy tính của mỗi người khác nhau (khác phiên bản Python, khác phiên bản database, thiếu thư viện hệ thống).
...
## 4️⃣ Các Tình Huống Thực Chiến Của Docker
...
- **Giải pháp:** Vào năm 2023, công cụ **OrbStack** đang nổi lên như một vị cứu tinh thay thế hoàn hảo cho Docker Desktop trên Mac nhờ tốc độ khởi động chỉ 2 giây và cực kỳ tiết kiệm pin.
...
- ⬅️ **Bài trước:** [Cài Đặt Môi Trường Container & Kích Hoạt Docker Engine Trên Mọi OS](../../setup/install-docker.md)
- ➡️ **Bài tiếp theo:** [Các Lệnh Điều Khiển Image & Container Theo Vòng Đời](./01_images-and-containers.md)
...
- [Trang chủ tài liệu Docker chính thức](https://docs.docker.com/) — nguồn tham khảo gốc đầy đủ nhất.
- [Play with Docker](https://labs.play-with-docker.com/) — môi trường thực hành Docker online miễn phí, không cần cài đặt.
...
- **v3.0.0 (26/05/2026)** — Chuẩn hoá Alert Box sang định dạng GitHub; tinh chỉnh H1 và các H2 thành dạng câu hỏi mở; nâng cấp văn phong dẫn dắt.
- **v3.1.0 (01/06/2026)** — Điều chỉnh một số điểm không hợp lý (Thay đổi tiêu đề bài H1 để phù hợp hơn; Tách subgraph Kien Truc VM và subgraph Kien Truc Docker Container ra 2 dòng cho dễ nhìn - do cũ nằm ngang cùng 1 dòng khiến dễ xem hơn; Không quá áp đặt thời gian và thời gian sai vào bài; Thêm gợi ý chủ đề liên quan)
- **v3.1.1 (01/06/2026)** — Sửa lỗi QA: đồng bộ phiên bản metadata với changelog (v3.0.0 → v3.1.1); sửa lỗi lặp từ ở phần dẫn dắt; sửa typo "gọi ý" → "gợi ý"; bổ sung hướng `LR` cho sơ đồ mermaid đầu tiên để render ổn định và nhất quán với các sơ đồ khác.
