...
## 💡 Khi Máy Tính Trở Thành "Bãi Chiến Trường"

Hãy hình dung bạn là một nhà phát triển phần mềm năng nổ. Để phục vụ học tập và làm việc, máy tính của bạn được cài đặt trực tiếp (global install) hàng chục dịch vụ nặng nề:
...
Sau vài tháng, máy tính bắt đầu có dấu hiệu quá tải, RAM luôn bị chiếm dụng ngay cả khi không làm việc vì các dịch vụ ngầm tự động khởi động cùng hệ thống. Cực đoan hơn, khi bạn muốn nâng cấp PostgreSQL v14 lên v16, các xung đột cổng kết nối (Port 5432) và file cấu hình nổ ra liên miên. Hệ điều hành gốc bị "bẩn" hoàn toàn bởi hàng tá thư viện rác còn sót lại sau những lần cài đặt và gỡ bỏ.

Để giải phóng bạn khỏi "bãi chiến trường" lộn xộn này, **Công nghệ Container (Docker)** ra đời.
...
```bash
docker --version
# Kết quả mong đợi: Docker version 26.x.x hoặc mới hơn
```
...
# Lưu ý: cask đã được đổi tên thành "docker-desktop"; tên cũ "docker" vẫn còn nhưng nên dùng tên mới
...
# Xóa toàn bộ container đã dừng, network không dùng và TẤT CẢ image không gắn với container nào
# (Lưu ý: cờ -a xóa mọi image unused, không chỉ image mồ côi/dangling — cân nhắc trước khi chạy)
...
- **Trên macOS:** Chọn **Troubleshoot (Hình biểu tượng cái phao cứu sinh)** trên giao diện Docker Desktop -> Chọn **Uninstall** -> Kéo ứng dụng Docker thả vào Thùng rác. Chạy lệnh sau trên Terminal để xóa file rác:
    ```bash
    rm -rf ~/.docker
...
Môi trường Container đã sẵn sàng! Bước vào bài học đầu tiên để làm chủ công nghệ này:

- 📖 [Bài 00: Docker là gì?](../lessons/01_basic/00_what-is-docker.md)
- 📖 [Bài 01: Các lệnh điều khiển Image & Container](../lessons/01_basic/01_images-and-containers.md)
...
- **v1.0.0 (16/05/2026)** — Bản đầu tiên: hướng dẫn cài đặt Docker Desktop, Engine trên các OS.
- **v2.0.0 (26/05/2026)** — Chuẩn hoá H1 và metadata; mở bài bằng tình huống cài database thủ công vất vả và ẩn dụ container di động; đổi H2 sang dạng câu hỏi mở; chuẩn hoá Alert Box GitHub; Việt hoá comment trong code cài đặt Linux và chạy thử Nginx; bổ sung đánh giá công cụ thay thế OrbStack/Podman.
- **v2.0.1 (01/06/2026)** — Sửa lỗi QA: cập nhật cask Homebrew `docker` → `docker-desktop` (tên mới chuẩn); sửa comment sai về `docker system prune -a` (xóa mọi image unused, không chỉ dangling); đồng bộ phiên bản kỳ vọng `docker --version` (25.x → 26.x) cho nhất quán; đổi "On macOS" → "Trên macOS"; dọn khoảng trắng thừa cuối lệnh `newgrp docker`.
