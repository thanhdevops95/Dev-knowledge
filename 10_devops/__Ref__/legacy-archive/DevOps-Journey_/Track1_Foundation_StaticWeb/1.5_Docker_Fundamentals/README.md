...
>
> You are developing a Python app. It works perfectly on your laptop. You send it to your colleague. It crashes because they have Python 3.8 and you have 3.9. They fix it. Then you deploy to the server. It crashes again because the server is missing a C++ library. You spend 3 days debugging "Environment Hell".
>
> *Bạn viết app Python. Chạy ngon trên laptop. Gửi cho đồng nghiệp thì crash do họ dùng khác version Python. Họ sửa xong, deploy lên server lại crash vì thiếu thư viện C++. Bạn mất 3 ngày chỉ để fix lỗi môi trường.*
>
> **Docker solves this by packaging the App + Libraries + OS Config into a single box (Image), guaranteed to run the same everywhere!**
>
> *Docker đóng gói App + Thư viện + Cấu hình OS vào một hộp duy nhất (Image), đảm bảo chạy giống hệt nhau mọi nơi!*
...
#### 1.4 Deep Dive: Why is it light? (Tại sao Container nhẹ?)

**The Secret: The Shared Kernel (Bí mật: Chia sẻ nhân Kernel)**

- **Virtual Machine (VM):** Each VM has its FULL Operating System (kernel + hardware drivers).
  *(Mỗi VM chứa trọn vẹn một Hệ điều hành riêng (nhân + driver phần cứng).)*
- **Container:** All containers share the **Host OS Kernel**. They only contain the application and user-space libraries (bins/libs).
  *(Tất cả container dùng chung nhân Kernel của máy chủ. Nó chỉ chứa app và thư viện cần thiết.)*

> 💡 **Analogy (Phép ẩn dụ):**
> - **VM** is like a **House**: It has its own plumbing, electricity, and foundation (Kernel). Building 10 houses costs a lot.
> - **Container** is like an **Apartment**: It shares the plumbing and foundation (Kernel) of the building but has its own private walls (Namespaces).
>
> *(VM giống như Ngôi nhà riêng: Có móng, điện nước riêng. Xây 10 nhà rất tốn kém. Container giống như Căn hộ chung cư: Dùng chung móng và điện nước tòa nhà, nhưng có tường riêng biệt.)*
...
> 💡 **Analogy:**
> - **Client (Remote)**: Like the TV Remote. You press buttons here.
> - **Daemon (Brain)**: Like the TV itself. It does the work (shows picture).
> - *You can run the Client on your Laptop (Windows) but control the Daemon on a Server (Linux)!*
...
> 💡 **Analogy:** Think of an Image like a **Photoshop Project** with many layers.
> - Layer 1: Background (OS - Ubuntu)
> - Layer 2: Shape (Library - Python)
> - Layer 3: Text (App Code)
> When you look from the top, you see the final picture. If you change the Text (Layer 3), you don't need to repaint the Background (Layer 1).
>
> *(Image giống như file Photoshop nhiều lớp. Nếu bạn sửa lớp trên cùng, các lớp dưới không cần vẽ lại -> Tiết kiệm thời gian build.)*
...
#### 4.2 Deep Dive: The "PID 1" Mystery (Bí ẩn PID 1)

> ❓ **Question:** Why does my container start and then immediately exit? (`docker run ubuntu`)
> *(Tại sao container khởi động xong tắt ngay lập tức?)*

**Answer:** A Container is NOT a Virtual Machine. It is just **ONE Process**.
*(Container KHÔNG PHẢI máy ảo. Nó chỉ là MỘT Tiến Trình.)*

- When a container starts, Docker runs the command (PID 1).
- **Rule:** **If PID 1 dies, the Container dies.** *(Nếu tiến trình số 1 chết, Container chết theo.)*
- `docker run ubuntu` runs `bash`. Without a terminal attached, `bash` has nothing to do -> exits -> Container stops.
- *Fix:* Keep it alive with a long-running process or interactive mode (`-it`).

#### 4.3 Managing Containers (Quản lý containers)
...
#### 4.4 Interacting with Containers (Tương tác với container)
...
#### 4.5 Container Lifecycle (Vòng đời Container)
...
> 📖 **Real-world Scenario:**
>
> You start a Postgres container. You save 1000 user records. You delete the container to fix a config.
> **RESULT:** All 1000 records are deleted instantly. You just lost production data.
>
> *(Bạn xóa container để sửa lỗi. Mất sạch 1000 bản ghi khách hàng. Thảm họa.)*
>
> **Solution:** **Volumes** (The "USB Stick").
>
> 💡 **Analogy:** Volume is like a **USB Stick**. Even if your laptop stays on fire (Container deleted), the data on the USB stick (Volume) is safe. You can plug it into a new laptop.
...
> 📖 **Real-world Scenario:**
>
> Web App needs to talk to Database. Database IP changes every time it restarts (`172.17.0.2` -> `172.17.0.5`).
> **Problem:** Web App crashes because it can't find Database.
>
> **Solution:** **Docker DNS**. Use a custom network, and they can call each other by **NAME** (`db_host`) instead of IP.
>
> 💡 **Analogy (Virtual Switch):** Creating a network is like buying a **Private WiFi Router**. Only devices connected to this WiFi can talk to each other. They see each other by name.
