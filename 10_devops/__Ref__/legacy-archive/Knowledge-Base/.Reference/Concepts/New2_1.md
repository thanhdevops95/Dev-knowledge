...
bash# Tạo file README trong thư mục gốc
cd todo-app-devsecops
Tạo file README.md với nội dung:
markdown# TODO App - DevSecOps Learning Project
...
bash# Cài Gin framework (web framework cho Go)
...
gorouter.Use(cors.New(cors.Config{...}))
...
gorouter.GET("/health", healthCheck)
router.POST("/ping", handlePing)
router.GET("/stats", getStats)
...
bash# Test 3: Ping nhiều lần
curl -X POST http://localhost:8081/ping
curl -X POST http://localhost:8081/ping
curl -X POST http://localhost:8081/ping
bash# Test 4: Lấy stats
...
Giải pháp:
bashgo mod tidy  # Tải về các dependencies còn thiếu
```
...
bash# Di chuyển vào thư mục python-service
cd ../python-service
...
pythonCORS(app, resources={r"/api/*": {"origins": "*"}})
...
Route decorator:
python@app.route('/api/ping', methods=['POST'])
def ping():
    ...
...
pythongo_data = response.json()

Parse JSON response từ Go
Tự động convert → Python dict

Error handling:
pythonexcept requests.exceptions.ConnectionError:
    ...

Bắt lỗi kết nối (Go service không chạy)

pythonexcept requests.exceptions.Timeout:
    ...
...
Response:
pythonreturn jsonify({...}), 200

Trả về JSON response với HTTP status code 200

3.5. Chạy Python Service
bash# Đảm bảo đang trong thư mục python-service/ và venv đã activate
python app.py
```
...
bash# Test 1: Health check Python
...
bash# Test 2: Test connectivity Python → Go
...
bash# Test 3: Ping qua Python Gateway
...
bash# Test 4: Lấy stats qua Python
...
Giải pháp:
bash# Kiểm tra venv đã activate chưa
...
Giải pháp:
bash# Kiểm tra Go service có chạy không:
curl http://localhost:8081/health
...
bash# Ping trực tiếp Go (bypass Python)
curl -X POST http://localhost:8081/ping
...
bash# Tắt Go service (Ctrl+C trong terminal Go)
...
pythonlogger.info("Received ping request from user")
Bước 3: Python forward sang Go
pythonresponse = requests.post(f"{GO_SERVICE_URL}/ping", timeout=5)
...
goc.JSON(http.StatusOK, gin.H{"message": "Pong", ...})
Bước 6: Python nhận response, parse JSON
pythongo_data = response.json()
Bước 7: Python trả về User
pythonreturn jsonify({'go_response': go_data, ...}), 200
...
Code Go:
gofunc resetStats(c *gin.Context) {
    mutex.Lock()
    defer mutex.Unlock()
...
Code Python:
python@app.route('/api/reset', methods=['POST'])
def reset_stats():
    try:
...
Kiểm tra:
bash# Kiểm tra có bao nhiêu process Go đang chạy
# Windows
tasklist | findstr go
