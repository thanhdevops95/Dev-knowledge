...
**OTel** = CNCF standard (CNCF incubating). Vendor-neutral instrumentation.
...
Trước OTel, mỗi vendor (Jaeger, Zipkin, Datadog, New Relic) có SDK riêng. Đổi vendor = viết lại toàn bộ instrumentation. OTel là **chuẩn CNCF** (CNCF incubating) giải bài này — instrument 1 lần, đổi backend bao nhiêu lần cũng được. Đây là lý do 2026 OTel là default lựa chọn:
...
- **v1.1.3 (13/06/2026)** — Sửa lỗi factual: OTel "graduated 2024" → "CNCF incubating" (2 chỗ); ví dụ `traceparent` trace-id sửa từ 33 hex → đúng 32 hex (giá trị chuẩn W3C).
